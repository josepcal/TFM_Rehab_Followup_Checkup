...
- **Estado:** Aceptada
- **Contexto:** La extracción de métricas y la llamada al LLM no deben bloquear la API.
- **Decisión:** Worker como contenedor aparte; cola ligera (Redis + RQ o tabla de jobs en Postgres con `SKIP LOCKED`). Ejecución con timeout y captura de errores.
- **Consecuencias:** El insight de IA es opcional y asíncrono; los fallos se registran como estado de error (ver ADR-0009).
