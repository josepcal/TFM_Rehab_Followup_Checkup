...
: "${MINIO_APP_USER:?Falta MINIO_APP_USER en .env}"
: "${MINIO_APP_PASSWORD:?Falta MINIO_APP_PASSWORD en .env}"
...
echo "------------------------------------------------------------"
echo " Variables API (usuario scoped, SIN privilegios admin):"
echo "   STORAGE_BACKEND=s3"
echo "   S3_ENDPOINT_URL=http://localhost:$MINIO_API_PORT"
echo "   S3_ACCESS_KEY_ID=$MINIO_APP_USER"
echo "   S3_SECRET_ACCESS_KEY=<MINIO_APP_PASSWORD>"
echo "   S3_BUCKET=$MINIO_BUCKET"
echo "   S3_REGION=$MINIO_REGION_NAME"
echo "   S3_FORCE_PATH_STYLE=true"
echo "------------------------------------------------------------"
echo " Admin (solo consola/gestion — NO usar en la API):"
